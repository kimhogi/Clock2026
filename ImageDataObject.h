...
struct STG_PATH{
	IStorage* pStorage;
	CString	strPath;
	STG_PATH(): pStorage(NULL){}
	STG_PATH( IStorage* pStg, CString str): pStorage(pStg), strPath( str){}
	STG_PATH( const STG_PATH& rhs)
	{
		pStorage = rhs.pStorage;
		// 061227 KBB if( pStorage)
		// 061227 KBB pStorage->AddRef();		
		strPath = rhs.strPath;
	}
	~STG_PATH(){
		// 061227 KBB if( pStorage)
		// 061227 KBB pStorage->Release();
	}
	void operator=( const STG_PATH& rhs)
	{
		if( this != &rhs)
		{
			pStorage = rhs.pStorage;
...
typedef std::vector< STG_PATH> SPVEC;
...
public:
	...
	//
	// 061227 KBB static void InsertBitmap(IRichEditOle* pRichEditOle, HBITMAP hBitmap);
	static void InsertBitmap(IRichEditOle* pRichEditOle, HBITMAP hBitmap, CString strPath);
	static SPVEC m_vecStoragePath;
